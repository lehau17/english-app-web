...
export const CreatePodcastPageFinal: React.FC = () => {
  ...
  const handleGenerateAudio = async () => {
    ...
    const speechSpeed = watch('speechSpeed')
    ...
    try {
      // Mock API call - replace với actual API
      const response = await new Promise<{
        ...
        setTimeout(() => {
          resolve({
            success: true,
            audioUrl: `data:audio/wav;base64,mock-generated-audio-${Date.now()}`,
            duration: 120,
          })
        }, 2000)
      })
      ...
      if (response.success && response.audioUrl) {
        ...
        setValue('audioUrl', response.audioUrl)
        setAudioDuration(response.duration || 0)
      } else {
        alert('Tạo audio thất bại. Vui lòng thử lại.')
      }
    } catch (error) {
